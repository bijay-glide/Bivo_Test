...
/**
 * MultiCurrencyAccountPage
 *
 * Covers the bu-web dashboard "Add a New Account" multicurrency modal
 * (multicurrency-add-account-button → addcurrencymodal), which can open both:
 *   - Fiat Currency accounts (Fiat tab)  → POST /business/v1/business/wallet-account
 *   - Stablecoin accounts    (Crypto tab) → POST /business/v1/coin/accounts
 *
 ...
 *
 * The whole modal is keyed by stable data-testids, so locators target those.
 */
class MultiCurrencyAccountPage {
  ...
  async createFiatAccount(currencyCode, accountName) {
    ...
    const responsePromise = this.page.waitForResponse(
      (r) => r.url().includes('/business/v1/business/wallet-account') && r.request().method() === 'POST',
      { timeout: 20000 },
    );
  ...
  async createStablecoinAccount(coinCode, accountName) {
    ...
    const responsePromise = this.page.waitForResponse(
      (r) => r.url().includes('/business/v1/coin/accounts') && r.request().method() === 'POST',
      { timeout: 20000 },
    );
    await this.confirmButton.click();
    const response = await responsePromise;
...
module.exports = MultiCurrencyAccountPage;
