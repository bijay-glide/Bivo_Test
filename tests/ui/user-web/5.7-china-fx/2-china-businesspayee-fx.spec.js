// User-web FX — China, business payee. Only one extra step versus the individual-payee
// channel coverage in 1-china-fx.spec.js: switching to the "Business" tab on the Create
// FX Transaction screen before selecting the destination country (same tab switch
// pattern already used elsewhere in user-web, e.g. 5.9-vietnam-fx / 5.10-elsalvador-fx's
// business files). Everything else — amount entry, banking, review, confirm, ledger
// verification — reuses the same FxTransactionPage methods as the individual-payee file.
//
...
// flow too — CNY defaults to Bank Deposit here (individual defaults to Alipay).
...
test.describe('User-web FX — China, business payee', () => {
  test('Sends a new FX transaction to a China business payee via Bank Deposit (CNY default)', async ({ page, request }) => {
  ...
  test('Sends a new FX transaction to a China business payee via Instant Card Payout (CNY)', async ({ page, request }) => {
    test.setTimeout(180000);
  ...
  test('Sends a new FX transaction to a China business payee via Instant Card Payout (CNH default)', async ({ page, request }) => {
    test.setTimeout(180000);
    ...
    await test.step('Step 9 | Review screen, note, and invoice number', async () => {
      await fxPage.verifyReviewTransferScreenShowsName(businessName);
      await fxPage.fillFxPaymentNote(fxData.note);
      await fxPage.fillFxInvoiceNumberIfPresent();
    });
  ...
  test('Sends a new FX transaction to a China business payee via Bank Deposit (CNH)', async ({ page, request }) => {
    test.setTimeout(180000);
