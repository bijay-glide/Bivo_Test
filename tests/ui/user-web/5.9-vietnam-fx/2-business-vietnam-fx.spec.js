// User-web FX — Vietnam, business payee. Per a live probe, the Business tab exposes only
// 2 deliver-to channels for Vietnam — Bank Deposit (default) and Instant Card Payout — no
...
//
// To implement either: remove `.fixme`, then follow the Philippines business suite as the
// template — switchToBusinessTab, selectBusinessDestinationCountryByTestId, amount,
...
test.describe('User-web FX — Vietnam, business payee', () => {
  test.fixme('Sends a new FX transaction to a Vietnam business payee via Bank Deposit', async ({ page, request }) => {
  ...
  test.fixme('Sends a new FX transaction to a Vietnam business payee via Instant Card Payout', async ({ page, request }) => {
    const fxPage = new FxTransactionPage(page);
    const userData = resolveUserDataForLogin();
    await loginUserWebWithPhone({ page, request, userData });
    await fxPage.navigateToCreateFxTransactionUserWeb();
