// Bu-web FX — United Kingdom payee lifecycle. Three scenarios exercising saved-payee
...
test.describe('Bu-web FX — United Kingdom payee lifecycle', () => {
  test.describe.configure({ retries: 1 });

  test('Reuses a GB payee created via IBAN for a second send via Bank Deposit', async ({ page }) => {
  ...
  test('Edits a saved GB payee from the standalone Payees list page, then sends to the edited payee', async ({ page }) => {
    test.setTimeout(180000);
  ...
  test('Edits a saved GB payee from the transaction Review Transfer page, then confirms with the edit', async ({ page }) => {
    test.setTimeout(180000);
