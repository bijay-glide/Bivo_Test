// Bu-web FX — China (individual payee) lifecycle. Three scenarios exercising
// saved-payee reuse/edit on top of the channel coverage in 1-china-individualpayee-fx.spec.js:
...
test.describe('Bu-web FX — China payee lifecycle', () => {
  test.describe.configure({ retries: 1 });

  test('Reuses a China payee created via UnionPay for a second send via Bank Deposit', async ({ page }) => {
  ...
  test('Edits a saved China payee from the standalone Payees list page, then sends to the edited payee', async ({ page }) => {
    test.setTimeout(180000);
  ...
  test('Edits a saved China payee from the transaction Review Transfer page, then confirms with the edit', async ({ page }) => {
    test.setTimeout(180000);
