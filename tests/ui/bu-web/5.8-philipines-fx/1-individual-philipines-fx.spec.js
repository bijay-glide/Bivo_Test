// Bu-web FX — Philippines. Bank Deposit is the default deliver-to channel selected
...
// same convention as 5.4-uk-fx's BIVO_PREFIX / 5.7-china-fx's BIVO_PREFIX.
...
// Sandbox card-vault test PAN, shared with 5.4-uk-fx / 5.5-india-fx's Instant Card
...
test.describe('Bu-web FX — Philippines', () => {
  test('Sends a new FX transaction to Philippines via Bank Deposit', async ({ page }) => {
  ...
  for (const provider of MOBILE_WALLET_PROVIDERS) {
    test(`Sends a new FX transaction to Philippines via Mobile Wallet (${provider.label})`, async ({ page }) => {
      test.setTimeout(180000);
  ...
  for (const provider of MOBILE_WALLET_ADDITIONAL_PROVIDERS) {
    test(`Sends a new FX transaction to Philippines via Mobile Wallet -Additional (${provider.label})`, async ({ page }) => {
      test.setTimeout(180000);
  ...
  test('Sends a new FX transaction to Philippines via Instant Card Payout', async ({ page }) => {
    test.setTimeout(180000);
