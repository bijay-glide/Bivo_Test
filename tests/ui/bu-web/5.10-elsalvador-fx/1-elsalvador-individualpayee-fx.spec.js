// Bu-web FX — El Salvador. BCR Pay is the default deliver-to channel selected
...
// Business tab has not been recorded at all — see 2-elsalvador-business-fx.spec.js.
...
test.describe('Bu-web FX — El Salvador', () => {
  test('Sends a new FX transaction to El Salvador via BCR Pay', async ({ page }) => {
  ...
  test('Sends a new FX transaction to El Salvador via Bank Deposit', async ({ page }) => {
    test.setTimeout(180000);
  ...
  test.fixme('Sends a new FX transaction to El Salvador via Instant Card Payout', async ({ page }) => {
    const fxPage = new FxTransactionPage(page);
    const userData = resolveBuWebUserDataForLogin();
    await loginBuWebWithEmail({ page, userData });
    await fxPage.navigateToCreateFxTransactionUserWeb();
