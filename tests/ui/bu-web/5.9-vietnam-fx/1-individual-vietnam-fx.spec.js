// Bu-web FX — Vietnam. Bank Deposit is the default deliver-to channel selected
// immediately after choosing Vietnam as the destination, under VND. Structured the same
// way as 5.8-philipines-fx: same login/summary/review/confirm/ledger validations, just
...
// just the shared lead-in wired up (see the `if (!sc.ready)` pattern already used by
// 5.4-uk-fx's matrix). The Business tab (Bank Deposit + Instant Card Payout, per a live
// probe) belongs in 2-business-vietnam-fx.spec.js once recorded.
...
// live at exactly $25 — same convention as 5.8-philipines-fx's SEND_AMOUNT_USD.
const SEND_AMOUNT_USD = '25';

// Static prefix keeps generated banking fields recognisable as automated test data —
// same convention as 5.8-philipines-fx's BIVO_PREFIX.
...
test.describe('Bu-web FX — Vietnam', () => {
  test('Sends a new FX transaction to Vietnam via Bank Deposit', async ({ page }) => {
  ...
  for (const provider of MOBILE_WALLET_PROVIDERS) {
    test(`Sends a new FX transaction to Vietnam via Mobile Wallet (${provider.label})`, async ({ page }) => {
      test.setTimeout(180000);
  ...
  test.fixme('Sends a new FX transaction to Vietnam via Instant Card Payout', async ({ page }) => {
    const fxPage = new FxTransactionPage(page);
    const userData = resolveBuWebUserDataForLogin();
    await loginBuWebWithEmail({ page, userData });
    await fxPage.navigateToCreateFxTransactionUserWeb();
